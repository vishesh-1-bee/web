...
const Websitesection = ({ generatedCode }: Props) => {
  ...
  // Inject HTML + enable element selection
  useEffect(() => {
    const iframe = iframeRef.current;
    if (!iframe) return;
  ...
  return (
    <div className="flex gap-2 w-full">
      ...
      {/* settings panel */}
      {/* <ElementSettingSection element={selectedelement} /> */}

      {selectedelement?.tagName=='IMG' ?
      // @ts-ignore
      <Imagesetting selectedEl={selectedelement}/>:
      selectedelement?
      // @ts-ignore
      <ElementSettingSection  element={selectedelement} clearSelectiom={()=>{setselectedelement(null)}}/>:null}
    </div>
  );
};
